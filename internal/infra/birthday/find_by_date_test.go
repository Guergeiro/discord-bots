...
func TestFindByDate(t *testing.T) {
	...
	t.Run("valid find all", func(t *testing.T) {
		...
		birthdays := []entity.Birthday{
			entity.NewBirthday(id, date),
			entity.NewBirthday(id, date.AddDate(0, 1, 0)),
		}
		...
		output, err := repo.FindByDate(context.Background(), date)

		assert.NoError(t, err)
		assert.Len(t, output, 1)

		actualBirthday := output[0]
		assert.Equal(t, birthdays[0].Id, actualBirthday.Id)
		assert.Equal(t, birthdays[0].PrettyBirthday(), actualBirthday.PrettyBirthday())
	})
	...
	t.Run("error in query", func(t *testing.T) {
		...
		output, err := repo.FindByDate(context.Background(), time.Now())
